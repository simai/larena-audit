...
The canonical integration also removes the reverse runtime dependency from
`larena/audit` to `larena/admin`: Audit no longer registers an Admin navigation
contributor from its service provider. The equivalent integration contributor
is owned and registered by `larena/admin`, while its descriptor continues to
identify `larena/audit` as the product owner.
